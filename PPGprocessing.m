...
% [ nD , nA , nB ]    =   pulseDelineation ( PPGGreen , fo , Setup );
...
%xline(nD,'color',[0.35 0.35 0.35],'LineWidth',0.5,'HandleVisibility','off');
title('PPGRED');
%Claculamos la diferencia entre el maximo y el minimo de cada latido:
%diffMaxMinRed=FiltroPBajoRoja(1+round(nA(~isnan(nA))*fo))- FiltroPBajoRoja(1+round(nB(~isnan(nB))*fo));
...
%los latidos
%IR
maxIR = (iA>(40*60) & iA<(60*60));
maxIR = find(maxIR==1);
iA=iA(maxIR(1):maxIR(end));

minIR = (iB>(40*60) & iB<(60*60));
minIR = find(minIR==1);
iB=iB(minIR(1):minIR(end));

%Red
maxRed = (nA>(40*60) & nA<(60*60));
maxRed = find(maxRed==1);
nA=nA(maxRed(1):maxRed(end));

minRed = (nB>(40*60) & nB<(60*60));
minRed = find(minRed==1);
nB=nB(minRed(1):minRed(end));

clear minRed minIR maxIR maxRed

latidos = min(length([nA, nB, iA, iB]));
Value=nan(latidos,1);

MaxsIr=table(iA, Value, 'VariableNames', {'Time', 'Value'});
MinsIr=table(iB, Value, 'VariableNames', {'Time', 'Value'});
MaxsR=table(nA, Value, 'VariableNames', {'Time', 'Value'});
MinsR=table(nB, Value, 'VariableNames', {'Time', 'Value'});
clear Value  %iA iB nA nB
...
plot(MaxsR.Time(1:end-1),R+91,'DisplayName', 'R+91');
legend
...
%Media con 3 latidos
% for i=1:length(picosRed)-2
%     %Buscamos el pico y hacemos una media del valor de la señal hasta el
%     %siguiente
%     red_dc(j) = mean(apnea.RC(find(apnea.Time==picosRed(i)):find(apnea.Time==picosRed(i+2))));
%     ir_dc(j) = mean(apnea.IC(find(apnea.Time==picosInfra(i)):find(apnea.Time==picosInfra(i+2))));
% 
%     red_ac(j) = max(apnea.RC(find(apnea.Time==picosRed(i)):find(apnea.Time==picosRed(i+2))))-min(apnea.RC(find(apnea.Time==picosRed(i)):find(apnea.Time==picosRed(i+2))));
%     ir_ac(j) = max(apnea.IC(find(apnea.Time==picosInfra(i)):find(apnea.Time==picosInfra(i+2))))-min(apnea.IC(find(apnea.Time==picosInfra(i)):find(apnea.Time==picosInfra(i+2))));
% 
%     a=(red_ac(j)/red_dc(j));
%     b=(ir_ac(j)/ir_dc(j));
%     c=a/b;
...
%media con 3
% for i=1:length(nPicosRed)-2
%     %Buscamos el pico y hacemos una media del valor de la señal hasta el
%     %siguiente
%     n_red_dc(k) = mean(normal.RC(find(normal.Time==nPicosRed(i)):find(normal.Time==nPicosRed(i+2))));
%     n_ir_dc(k) = mean(normal.IC(find(normal.Time==nPicosInfra(i)):find(normal.Time==nPicosInfra(i+2))));
% 
%     n_red_ac(k) = max(normal.RC(find(normal.Time==nPicosRed(i)):find(normal.Time==nPicosRed(i+2))))-min(normal.RC(find(normal.Time==nPicosRed(i)):find(normal.Time==nPicosRed(i+2))));
%     n_ir_ac(k) = max(normal.IC(find(normal.Time==nPicosInfra(i)):find(normal.Time==nPicosInfra(i+2))))-min(normal.IC(find(normal.Time==nPicosInfra(i)):find(normal.Time==nPicosInfra(i+2))));
% 
...
Setup.plotflag      =   true;
plotflag = true;
[ nD , nA , nB ]    =   pulseDelineation ( PPGIR , fo , Setup );  % PREGUNTAR POR ESE REDONDE A FS SI AFECTA
% [ nD , nA , nB ]    =   pulseDelineation ( PPGsignal , fs , Setup );  % PREGUNTAR POR ESE REDONDE A FS SI AFECTA
